...
-- Fix flowers SELECT policy to PERMISSIVE
...
-- Fix orders SELECT policies to PERMISSIVE
...
-- Fix order_items SELECT policies to PERMISSIVE
...
CREATE POLICY "Shop owners can read their order items"
  ...
  USING (EXISTS (
    ...
    WHERE o.id = order_items.order_id AND owns_shop(auth.uid(), o.shop_id)
  ));
...
-- Fix flowers INSERT/UPDATE/DELETE to PERMISSIVE for shop owners and admins
...
-- Fix orders INSERT/UPDATE/DELETE to PERMISSIVE
...
-- Fix order_items INSERT to PERMISSIVE
